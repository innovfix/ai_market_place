...
interface HeaderSearchWithSuggestionsProps {
  query: string;
...
export function HeaderSearchWithSuggestions({ 
  query, 
  setQuery, 
  placeholder = "What service are you looking for today?",
  className 
}: HeaderSearchWithSuggestionsProps) {
  ...
  const handleSuggestionClick = (suggestion: SearchSuggestion) => {
    ...
    router.push(`/dashboard/search?q=${encodeURIComponent(suggestion.text)}`);
  };
        ...
        >
          <div className="max-h-80 overflow-y-auto">
            ...
            {filteredSuggestions.map((suggestion, index) => (
              <div
                key={suggestion.id}
                className={cn(
                  "flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors border-b border-gray-800 last:border-b-0",
          ...
          {!query.trim() && (
            <div className="border-t border-gray-700 px-4 py-3 bg-gray-800/50">
              <div className="text-xs font-medium text-gray-400 mb-2">Browse by category</div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="text-gray-300 hover:text-blue-400 cursor-pointer">AI Services</div>
                <div className="text-gray-300 hover:text-blue-400 cursor-pointer">Music & Audio</div>
                <div className="text-gray-300 hover:text-blue-400 cursor-pointer">Digital</div>
                <div className="text-gray-300 hover:text-blue-400 cursor-pointer">Video & Animation</div>
                <div className="text-gray-300 hover:text-blue-400 cursor-pointer">Writing & Translation</div>
                <div className="text-gray-300 hover:text-blue-400 cursor-pointer">Business</div>
              </div>
            </div>
          )}
        </div>
